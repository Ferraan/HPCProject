...
int main(int argc, char *argv[]){
    ...
    const int n=1000;  //Up to n=40000 using 16Gb RAM and a little bit of swap
    const int iter=1000;
    const int ch=1;
    ...
    for(int i = *mystart; i <= *myend; i++){
        ...
        int posx = i/n;
        int posy = i % n;
        double x0=coordX[posx];
        double y0=coordY[posy];
        double x2=0;
        double y2=0;
        double x=0;
        double y=0;
    ...
    if (myrank==0)
    {
        kGLO=(double*)calloc(n*n,sizeof(double));
        displ[0]=0;
        for (int i = 1; i < P; i++)
        {
            #ifdef DEBUG
    ...
    MPI_File fh;
    MPI_File_open(MPI_COMM_WORLD,"mpiwparallel.bin",MPI_MODE_WRONLY | MPI_MODE_CREATE,MPI_INFO_NULL,&fh);
    MPI_File_write_all(fh,kLOC,ntasks,MPI_DOUBLE,MPI_STATUS_IGNORE);
    free(mystart);
	free(myend);
    free(kLOC);
    free(kGLO);
